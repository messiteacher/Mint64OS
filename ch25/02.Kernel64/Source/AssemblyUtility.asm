...
kOutPortWord:
    push rdx            ; 함수에서 임시로 사용하는 레지스터를 스택에 저장
    push rax            ; 함수의 마지막 부분에서 스텍에 삽입된 값을 꺼내 복원
...
kReadTSC:
    ...
    shl rdx, 32         ; RDX 레지스터에 있는 상위 32비트 TSC 값과  RAX 레지스터에 있는 하위 32비트 TSC 값을 OR하여 RAX 레지스터에 64비트 TSC 값을 저장
    pop rdx
    ret
...
kSwitchContext:
    ...
    ; Current Context가 NULL이면 콘텍스트를 저장할 필요가 없음
    ...
    mov rax, qword[rbp + 8] ; RIP 레지스터를 Return Address로 설정하여 다음 콘텍스트 복원 시에 이 함수를 호출한 위치로 이동하게 함
    ...
    ; 가장 끝부분에 SS, RSP, RFLAGS, CS, RIP 레지스터를 저장했으므로, 이전 영역에 push 명령어로 콘텍스트를 저장하기 위해 스택을 변경
    add rdi, (19 * 8)
    mov rsp, rdi
    sub rdi, (19 * 8)
