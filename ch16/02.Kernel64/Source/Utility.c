...
void kCheckTotalRAMSize(void) {
    ...
    // 체크가 성공한 언드레스를 1MB로 나누어 MB 단위로 계산
...
int kHexToString(QWORD qwValue, char* pcBuffer) {
    ...
    // 버퍼에 1의 자리붙처 16, 256, ... 의 자리 순서로 숫자 삽입
    for (i = 0; qwValue > 0; i++) {
...
int kDecimalToString(long lValue, char* pcBuffer) {
    ...
    // 버퍼에 1의 자리부터 10, 100, 1000 ... 의 자리 순서로 숫자 삽이
    for ( ; lValue > 0; i++) {
    ...
    for (i = 0; i < iFormatLength; i++) {
        ...
        if (pcFormatString[i] == '%') {
            ...
            case 's':
                ...
                // 문자 출력
            case 'c':
                // 가변 인자에 들어 있는 파라미터를 문자 타입으로 변환하여 출력 버퍼에 복사하고 버퍼의 인덱스를 1만큼 이동
            ...
            case 'x':
            case 'X':
                // 가변 인자에 들어 있는 파리미터를 DWORD 타입으로 변환하여 출력 버퍼에 복사하고 출력한 길이만큼 버퍼의 인덱스를 이동
