...
void Main( void )
{
    ...
    kPrintString(0, 4, "Minimun Memory Size Check...................[    ]");
...
BOOL kInitializeKernel64Area(void) {
    ...
    while ((DWORD)pdwCurrentAddress < 0x600000) {
        *pdwCurrentAddress = 0x00;
        ...
        // 0으로 저장한 후 다시 읽었을 때 0이 나오지 않으면 해당 어드레스를 사용하는데 문제가 생긴 것이므로 더 이상 진행하지 않고 종료
...
BOOL kIsMemoryEnough(void) {
    ...
    // 0x4000000(64MB)까지 로프를 돌면서 확인
    while ((DWORD)pdwCurrentAddress < 0x4000000) {
        *pdwCurrentAddress = 0x12345678;

        // 0x12345678로 저장한 후 다시 읽었을 때 0x12345678이 나오지 않으면 해당 어드레스를 사용하는데 문제가 생긴 것이므로 더 이상 진행하지 않고 종료
