...
void kDeviceNotAvailableHandler(int iVectorNumber) {
    ...
    // 현재 태스크가 FPU를 사용한 적이 있는 지 확인해 FPU를 사용한 적이 없다면 초기화하고, 사용한 적이 있다면 저장된 FPU 콘텍스트를 복원
...
void kHDDHandler(int iVectoNumber) {
    char vcBuffer[] = "[INT:  , ]";
    ...
    vcBuffer[5] = '0' + iVectoNumber / 10;
    vcBuffer[6] = '0' + iVectoNumber % 10;
    // 발생한 횟수 출력
    ...
    if (iVectoNumber - PIC_IRQSTARTVECTOR == 14) {
        // 첫번째 PATA 포트의 인터럽트 발생 여부를 TRUE로 설정
        kSetHDDInterruptFlag(TRUE, TRUE);
    }
    // 두 번쨰 PATA 포트의 인터럽트 벡터(IRQ 15) 처리
    else {
        // 두 번쨰 PATA 포트의 인터럽트 발생 여부를 TRUE로 설정
        kSetHDDInterruptFlag(FALSE, TRUE);
    }
    // EOI 전송
    kSendEOIToPIC(iVectoNumber - PIC_IRQSTARTVECTOR);
}