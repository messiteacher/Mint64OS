...
void kInitializeSerialPort(void) {
    ...
    // 통신 속도를 115200으로 설정
    // 라인 제어 레지스터(포트 0x3FB)의 DLAB 비트(비트 7)를 1로 설정하여 제수 레지스터에 접근
    ...
    // 송수신 방법을 설정
    // 라인 제어 레지스터(포트 0x3FB)에 통신 방법을 8비트, 패리티 없음(No Parity),
...
// 송신 FIFO가 비어 있는지를 반환
static BOOL kIsSerialTransmitterBufferEmtpy(void) {
    BYTE bData;

    // 라인 상태 레지스터(포트 0x3FD)를 읽은 뒤 TBE 비트(비트 1)를 확인하여 송신 FIFO가 비어 있는지 확인
...
void kSendSerialData(BYTE* pbBuffer, int iSize) {
    ...
    while (iSentByte < iSize) {
        ...
        while (kIsSerialTransmitterBufferEmtpy() == FALSE) {
            kSleep(0);
        }

        // 전송할 데이터 중에서 남은 크기와 FIFO의 최대 크기(16바이트)를 비교한 후 작은 것을 선택하여 송신 시리얼 포트를 채움
...
static BOOL kIsSerialRecevieBufferFull(void) {
    ...
    // 라인 상태 레지스터(포트 0x3FD)를 읽은 뒤 RxRD 비트(비트 0)를 확인하여 수신 FIFO에 데이터가 있는지 확인
    bData = kInPortByte(SERIAL_PORT_COM1 + SERIAL_PORT_INDEX_LINESTATUS);
...
void kClearSerialFIFO(void) {
    ...
    // 송수신 FIFO를 모두 비우고 버퍼에 데이터가 14바이트 찼을 때 인터럽트가 발생하도록 FIFO 제어 레지스터(포트 0x3FA)에 설정 값을 전송
