...
static void kSetUpTask(TCB* pstTCB, QWORD qwFlags, QWORD qwEntryPointAddress, void* pvStackAddress, QWORD qwStackSize) {
    ...
    // Return Address 영역에 kExit() 함수의 어드레스를 삽입하여 태스크의 엔트리 포인트 함수를 빠져나감과 동시에 kExitTask() 함수로 이동하도록 함
...
void kInitializeScheduler(void) {
    ...
    // 준비 리스트 와 우선순위별 실행 횟수를 초기화하고 대기 리스트도 초기화
...
BOOL kChangePriority(QWORD qwTaskID, BYTE bPriority) {
    ...
    // PIT 컨트롤러의 인터럽트(IRQ 0)가 발생하여 태스크 전환이 수행될 때 변경된 우선순위의 리스트로 이동
...
static TCB* kGetProcessByThread(TCB* pstThread) {
    ...
    // 내가 프로세스가 아니라면, 부모 프로세스로 설정된 태스크 ID를 통해 TCB 풀에서 태스크 자료구조 추출
        ...
        if (kGetListCount(&(gs_stScheduler.stWaitList)) >= 0) {
            while(1) {
                ...
                if (pstTask->qwFlags & TASK_FLAGS_PROCESS) {
                    // 프로세스를 종료할 때 자식 스레드가 존재하면 스레드를 모두 종료하고, 다시 자식 스레드 리스트에 삽입
                    ...
                    for (i = 0; i < iCount; i++) {
                        ...
                        // 자식 스레드 리스트에 연결도니 정보는 태스크 자료구조에 있는 stThreadLink의 시작 어드레스이므로, 태스크 자료구조의 시작
                        // 어드레스를 구하려면 별도의 계산이 필요함
                        pstChildThread = GETTCBFROMTHREADLINK(pstThreadLink);

                        // 다시 자식 스레드 리스트에 삽입하여 해당 스레드가 종료될 때 자식 스레드가 프로세스를 찾아 스스로 리스트에서 제거하도록 함
                    ...
                    // 아직 자식 스레드가 남아있다면 자식 스레드가 다 종료될 때까지 기다려야 하므로 다시 대기 리스트에 삽입
                ...
                kPrintf("IDLE: Task IDp[0x%q] is completely ended.\n", qwTaskID);
            }
        }
