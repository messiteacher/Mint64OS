...
BOOL kActivateKeyboard(void) {
    ...
    // ACK가 올 때까지 대기함
    // ACK가 오기 전에 키보드 출력 버퍼(포트 0x60)에 키 데이터가 저장되어 있을 수 있으므로 키보드에서 전달된 데이터를 최대 100개까지 수신하여 ACK를 확인
    for (j = 0; j < 100; j++) {
...
BOOL kChangeKeyboardLED(BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn) {
    ...
    // 키보드가 LED 데이터를 가져갔으므로 ACK가 올 떄까지 대기함
    for (j = 0; j < 100; j++) {
        for (i = 0; i < 0xFFFF; i ++) {
            // 출력 버퍼(포트 0x60)가 차 있으면 데이터를 읽을 수 있음
...
void UpdateCombinationKeyStatusAndLED(BYTE bScanCode) {
    ...
    // 눌림 또는 떨어짐 상태 처리, 최상위 비트(비트 7)가 1이면 키가 떨어졌음을 의미하고 0이면 눌림을 의미함
    if (bScanCode & 0x80) {
        bDown = FALSE;
        bDownScanCode = bScanCode & 0x7F;
    }
