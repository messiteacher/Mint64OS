...
int kSeekFile(FILE* pstFile, int iOffset, int iOrigin) {
    ...
    case FILESYSTEM_SEEK_CUR:
        // 이동할 오프셋이 음수이고, 현재 파일 포인터 값보다 크면 더 이상 갈 수 없으므로 파일의 처음으로 이동
    ...
    case FILESYSTEM_SEEK_END:
        // 이동할 옾셋이 음수이고, 현재 파일 포인터의 값보다 크면 더 이상 갈 수 없으므로 파일의 처음으로 이동
    ...
    // 이동하는 클러스터의 위치가 파일의 마지막 클러스터의 오프셋을 넘어서면 현재 클러스터에서 마지막까지 이동한 후 Write 함수를 이용해서 공백으로 나머지를 채움
    ...
    // 이동하는 클러스터의 위치가 현재 클러스터와 같거나 다음에 위치해 있다면 현재 클러스터를 기준으로 차이만큼만 이동하면 된다.
    ...
    //==================================================================================================
    // 실제 파일의 크기를 넘어서 파일 포인터가 이동했다면, 파일 끝에서부터 남은 크기만큼 0으로 채워줌
    if (dwLastClusterOffset < dwClusterOFfsetToMove) {
...
struct kDirectoryEntrySturct* kReadDirectory(DIR* pstDirectory) {
